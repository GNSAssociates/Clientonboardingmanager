...
function serviceTable(services: Array<{ name: string; price: number }>): string {
  const rows = services.map(s => `
    <tr>
      ...
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:13px;color:#374151;text-align:right;white-space:nowrap">£${s.price.toFixed(2)}/month</td>
    </tr>`).join('');
...
<table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;margin:16px 0">
  ...
  <tfoot>
    <tr style="background:#f0f4ff">
      ...
      <td style="padding:10px 12px;font-weight:700;font-size:14px;color:#1e3a8a;text-align:right">£${total.toFixed(2)}/month + VAT</td>
    </tr>
  </tfoot>
</table>`;
}
...
<div style="padding:24px 36px">
  ...
  <p style="margin:0 0 16px;font-size:14px;color:#4b5563;line-height:1.7">
    We are pleased to confirm that <strong>${d.firm.legalName}</strong> would like to act as accountants for
    <strong>${d.companyName}</strong> (Company No. ${d.companyNumber}). Please find below a summary of the
    agreed services and fees, together with your engagement letter to review and sign.
  </p>
  ${serviceTable(d.services)}
  <p style="margin:16px 0;font-size:13px;color:#6b7280">
    Annual equivalent: <strong>£${(total * 12).toFixed(2)} + VAT</strong> · Payment by GoCardless Direct Debit.
    <br><strong>Note: 20% VAT applies to all fees above.</strong>
  </p>
  <p style="margin:0 0 8px;font-size:14px;color:#374151">
    Please click the button below to read your full engagement letter, provide your previous accountant details,
    and sign the declaration. <strong>This link expires in 30 days (${d.expiresAt}).</strong>
  </p>
  ${actionButton(d.engagementUrl, 'Review &amp; Sign Engagement Letter', d.firm.accentColor)}
...
<div style="padding:24px 36px">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px">
    ${[
      ...
      ['Annual Revenue', `£${(total * 12).toFixed(2)} + VAT`],
