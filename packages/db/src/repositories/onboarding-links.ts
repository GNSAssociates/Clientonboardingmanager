...
/**
 * Insert or update an in-progress wizard draft, keyed by token. Drafts carry
 * status "draft" and never send email; the wizard state lives in letter_meta
 * (see letterMeta.wizardDraft). On the final "Generate & Send" the row is
 * promoted to status "sent" in-place, so progress is fully recoverable.
 */
export async function upsertDraftLink(
  tx: Tx,
  data: typeof onboardingLinks.$inferInsert,
): Promise<OnboardingLinkRow> {
  const [row] = await tx
...
export async function listDraftsByFirmSlug(tx: Tx, firmSlug: string) {
  return tx
    .select()
    .from(onboardingLinks)
    .where(and(eq(onboardingLinks.firmSlug, firmSlug), eq(onboardingLinks.status, "draft")))
    .orderBy(desc(onboardingLinks.updatedAt));
}
  ...
  const expiredLinks = await tx
    ...
    .where(
      and(
        ...
        lt(onboardingLinks.expiresAt, now),
      ),
    );
